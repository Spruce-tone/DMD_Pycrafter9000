...
## some self explaining functions
    ...
    def bmpload(self,image,size):
        ...
        for i in range(packnum):
            ...
            self.checkforerrors()
        print("Time for loading: ", time.time()-t)
...
##        arr.append(np.ones((1080,1920),dtype='uint8'))
        ...
        print ("Time for merging and encoding: ", time.time()-t)
        ...
        for i in range(int((num-1)//24)+1): #for i in range(len(encodedimages)) should work?
            ...
            logger.debug(f'encoded image : {len(encodedimages)} | size : {sizes}')
        ...
        while j <WIDTH:
            ...
            if i>0:
                if np.all(image[i,j,:]==image[i-1,j,:]):
                    ...
                    if j < WIDTH: #1919 since I compare j and j+1 pixel
    ...
    try:
        ...
        for file in sorted(os.listdir(directory)):
            ...
            if filename.endswith("320.png"):
                ...
                """
                arr = np.asarray(PIL.Image.open(directory_in_str+filename), dtype = np.bool)
                images.append(arr)
                i += 1
            if i > 1:
                break
